...
<script>
$(function(){
	$('.company-profile-section').css({"background":"url('<?php echo base_url() ?>/uploads/business/background/original/<?php echo $UserData[0]['background_img_url']; ?>')",'background-size': 'cover', 'background-repeat': 'no-repeat'});
})
</script>
  ...
  <div class="comviewbg">
	<div class="container">
    	<div class="company-profile-banner">
        	...
        	<h1><img class="b-logo" src="<?php echo base_url() ?>uploads/business/logo/<?php echo $UserData[0]['logo_url'];?>"></h1>
			<?php } else { ?>
			<h1><img class="b-logo" src="<?php echo base_url() ?>images/demo-logo150x150.jpg"></h1>
			<?php } ?>
			<p><?php echo $UserData[0]['description']; ?><?php echo anchor($UserData[0]['website_url'],'', array('target'=>'_blank')) ?></p>
        </div>
    </div>
	</div>
</div>
...
<section class="container-fluid detalle-container">
  <div class="container">
	...
	<?php if( !empty($getProposalCeo) ) { ?>
	<?php foreach($getProposalCeo as $getProposal) : ?>
<script>
$(function(){
    ...
    <div class="row commentarios-row">
      ...
      <div class="col-sm-12">
        <div class="col-sm-10">
          <div class="feed-user comentario-pro"> 
			<i style="background:url('<?php if(!empty($UserData[0]['ceo_profile_pic'])) { ?><?php echo base_url(); ?>uploads/profile/64/<?php echo $UserData[0]['ceo_profile_pic']; ?><?php } else { ?><?php echo base_url(); ?>images/user30x30.jpg<?php } ?>')"></i>	
			<span><?php echo ucfirst($UserData[0]['nickname']);?><font><?php echo ucwords($UserData[0]['first_name'].' '.$UserData[0]['last_name'] );?></font></span> 
		  </div>
      </div>
	  <div class="col-sm-2 feed-date">
		<?php 
			$actual_date = date("dS M Y", strtotime($UserData[0]['created_on'])); 
			$propdate = $getProposal->created_on;
      ...
      <div class="col-sm-12 feed-banners" id="bg<?php echo $getProposal->id; ?>">
        <div class="feed-banners-col">
				<?php if(!empty($UserData[0]['logo_url'])) { ?>
					<img width="150" src="<?php echo base_url(); ?>uploads/business/logo/150/<?php echo $UserData[0]['logo_url']; ?>" alt="">
				<?php } else { ?>
...
<section class="categories-row">
	 <div class="container">
        ...
        <div class="col-sm-12">
        <?php 
			if(!empty($getIndustriesCeo)) : 
				foreach ($getIndustriesCeo as $ind) : ?>
